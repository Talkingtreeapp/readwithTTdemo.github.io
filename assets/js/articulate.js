    ...
    var methods = {
        speak: function (options) {
            ...
            speech.onboundary = function (event) {
                var word = getWordAt(toSpeak, event.charIndex);

                if (options.onWordFound !== undefined) {
                    options.onWordFound({
        ...
        setVoice: function () {
            ...
            if (arguments[0] == "language") {
                ...
                if (requestedLanguage.length == 2) {
                    ...
                    for (var i = 0; i < voices.length; i++) {
                        if (voices[i].language.toUpperCase() == requestedLanguage) {
                            voiceUserDefault = voices[i].name;
                            console.log('lang', voiceUserDefault)
                            break
                        };
                    };
                }
